...
**기간**: 1~2주 (Hybrid 채택으로 단축)
**전제**: Phase 1 완료, Core 모듈 기능 확장 가능
**난이도**: Tier 3 (game 시스템 훅, Hybrid 로 일부 축소)

> **설계 변경 이력 (2026-04-22)**: Phase 1 역공학 (#11) 결과, 덕코프가 이미 `Duckov.EXPManager` 싱글턴과 `onLevelChanged` 이벤트를 보유함을 확인. PRD 초판의 "별도 궤도 XP 시스템" 가정을 재검토하고 **Hybrid 전략** 으로 확정 (Q-EXP-REUSE 해소).

## 6.0 Q-EXP-REUSE 결정 — Hybrid 전략

| 레이어 | 소유자 | 이유 |
|---|---|---|
| XP 수집 (적 처치·퀘스트·레이드) | **게임의 `EXPManager`** | 이미 모든 소스 훅 완료, 세이브 포함 |
| 레벨 계산 (threshold 테이블) | **게임의 `EXPManager.LevelFromExp`** | 이미 구현됨 |
| 레벨업 이벤트 | **게임의 `EXPManager.onLevelChanged`** | static Action 으로 노출됨 |
| **스탯 포인트 적립** | **`QuackForge.Progression`** | PRD 오리지널 기능 |
| **5-스탯 분배 (VIT/STR/AGI/PRE/SUR)** | **`QuackForge.Progression`** | PRD 오리지널 기능 |
| **스탯 적용 (MaxHP 증가 등)** | **`QuackForge.Progression` + Harmony** | PRD 오리지널 기능 |
| 세이브 | **`quackforge.json` 사이드카** | 우리 스탯 할당 상태만 |

### 결과

- **우리가 구현하지 않는 것**: XP 수집기, XpCurve, 레벨 계산, 적 티어별 XP 차등 (T2.4 XpCollector 삭제)
- **우리가 구현하는 것**: `EXPManager.onLevelChanged` 구독 → 스탯 포인트 +N → 유저 분배 → Harmony 로 Health/Stats 에 반영
- **상한**: 게임의 레벨 상한과 동일. 별도 관리 불필요 (결정 2-2 의 50 vs 100 논의 폐기 — 게임이 정한 값 사용)

## 6.1 Phase 2 목표

"게임이 제공하는 레벨업 이벤트를 받아서 QuackForge 스탯 포인트를 적립하고, 포인트를 스탯 5종(Phase 2 MVP 는 VIT 만)에 분배하면 자동으로 최대 HP 증가. 세이브 파일을 불러와도 스탯 할당 정보가 유지된다."

**MVP 수준**: UI 없음, 자동 VIT 적립 + HP 증가만. 수동 분배 UI 는 Phase 3.

## 6.2 Done 기준 (Hybrid 반영)

- [ ] `EXPManager.onLevelChanged` Harmony 구독 성공 (로그로 level N→N+1 표시)
- [ ] 레벨업 시 `StatPoints` +1 (설정 가능, 기본 1)
- [ ] MVP 기본 자동 분배: 누적 포인트 전부 `VIT` 로 자동 투입
- [ ] 투입된 `VIT` 1 포인트당 플레이어 최대 HP +10 (Harmony 로 `Health.MaxHealth` 계산식 확장)
- [ ] `quackforge.json` 사이드카에 스탯 할당 상태 저장·복원 (게임 XP 는 게임 세이브에 이미 있음)
- [ ] 레벨업 순간 콘솔 로그 + 간단 토스트 알림
- [ ] 디버그 명령: `AddXp(amount)` — 우리가 아닌 **게임 `EXPManager.AddExp`** 를 래핑해서 호출 (Phase 2 의 테스트 동력)

## 6.3 Scope

### In Scope
- `EXPManager.onLevelChanged` Harmony 구독 (XP 수집·레벨 계산은 게임에 위임)
- `StatPoints` 풀 적립
- 5-스탯 enum 정의 + `VIT` 만 실제 반영
- Health 패치로 `VIT × 10` 을 `MaxHealth` 에 가산
- `quackforge.json` 사이드카 IO
- 레벨업 알림 (로그 + 토스트)

### Out of Scope (Phase 3으로)
- 수동 스탯 분배 UI
- 나머지 4 스탯 (STR/AGI/PRE/SUR) 실제 반영
- 리스팩 (스탯 재분배)
- 레벨업 시각 효과 (파티클 등)
